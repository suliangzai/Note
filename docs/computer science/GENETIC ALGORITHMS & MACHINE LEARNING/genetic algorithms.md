...
2. 两栖动物： 半水栖，即可以生活在水中或陆地上；皮肤光滑；产卵

3. 爬行动物： 与两栖类一样为半水生动物；皮肤有鳞片；产卵；冷血

4. 鸟类： 会飞；产卵；温血动物

5. 哺乳动物：有毛或皮毛；有乳汁喂养幼崽；温血动物
...
设置 𝑗 = 0，并用随机值初始化![alt text](image-219.png)
...
对于左分支，我们搜索𝑥1 的最佳分割点。与上述方法相同，我们首先评估不同候选分割点的基尼不纯度，然后找出不纯度最低的一个。从下图可以看出，当使用 1.7803 
时，不纯度为 0.02
...
为了解决上述问题，我们可以使用随机森林，它是基于 Bagging（Bootstrap Aggregation 引导聚合的缩写）集合技术ensemble
technique开发的。
...
如果q=1，则距离度量称为曼哈顿或城市街区距离Manhattan or city
block distance。
...
在现实生活中，许多数据集都可以用高斯分布（单变量或多变量）来建模。因此，假设数据来自不同的高斯分布是非常自然和直观的。因此，我们可以尝试将数据集建模为多个高斯分布的混合物。这就是高斯混合模型Gaussian Mixture
Model（GMM）的核心思想。
...
步长大小包含在基因组中，并低于变异和选择本身： <x1 ,...,xn,σ>，突变步长大小不是由用户设定的，而是与解决方案共同演化的。不同的突变策略可能适用于进化搜索过程的不同阶段。

先变异 σ - 净变异效果： <x1 ,...,xn,σ>-> <x'1 ,...,x'n,σ'> 。顺序很重要：先变异 σ （参见后面的方法） - 然后变异 x ：x' = x + N(0,σ') 
...
父母：<x1 ,...,xn > 和 <y1 ,...,yn> ，随机选取一个基因 (k)， 孩子 1 是：
...
2. 从第一个交叉点开始，在 P2 的该段中寻找未被复制的元素 

3. 对于其中的每一个 i，在子代中寻找已从 P1 复制到其位置的元素 j 

4. 把 i 放到 P2 中 j 所占的位置，因为我们知道不会把 j 放到那里（因为子代中已经有了） 

5. 如果 P2 中 j 所占的位置已被子代 k 填入，则将 i 放入 P2 中 k 所占的位置 

6. 处理完交叉段的元素后，就可以从 P2 填入其余的子代了。第二个子代的创建过程类似
